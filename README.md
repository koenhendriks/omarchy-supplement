...
| `install-jira-cli.sh` | jira-cli, pointed at the work Jira with the `.env` token in `~/.netrc` |
...
the credential files each client needs, `install-omarchy-jira-plugin.sh` copies
`JIRA_API_TOKEN` into the plugin's entry in `shell.json`, and
`install-jira-cli.sh` writes it to `~/.netrc` as the password for the Jira host.
Nothing secret is committed. The Jira token is the one optional entry: without it
that plugin still opens tickets, it just stops looking anything up, and jira-cli
...
- **Prefer synthesised keystrokes over shelling out to `hyprctl`.**
  ...
  daemon.
- **jira-cli takes the token from the first of four places that answers, and the
  `.netrc` one matches on host *and* login.** The order is `$JIRA_API_TOKEN`, the
  config's own `api_token`, `~/.netrc`, then the keyring. `.netrc` is the only one
  that needs nothing exported per shell and survives a later `jira init`, which
  renames the old config to `.config.yml.bkp` and writes a fresh one from the API
  -- so an `api_token` put in the config disappears the first time the config is
  regenerated. The lookup is `machine == url.Host && login == login`, both exact,
  so an entry whose login is not the address in the config's `login` key is not a
  near miss: it is invisible, and the command fails as if unauthenticated.
  `install-jira-cli.sh` writes both from the same two constants for that reason.
- **`jira init` is fully non-interactive only once `--installation`, `--server`,
  `--login`, `--project` and `--board` are all passed.** It prompts for exactly
  what it was not given, and a missing one blocks a sourced `install-all.sh` run
  on a survey prompt with no way to answer it. `--force` covers the sixth prompt,
  the "config already exists, overwrite?" confirmation. `--board none` is a real
  answer meaning "no default board", not a board named none.
